...
tExec is a wrapper around exec from child_process with the nuance that it can execute a sequence of exec calls. This is useful when one wnats the execute multiple exec call in an order and have a spinner working on each call with out pauses.
Lets see a simple usage case:
...
This will execute a simple speel command for 3 seconds, the style will fallback to default as we didn't specify it (more on styling in styling section).
tExec takes two arguments a configs(array of config obejct) argument and an optional callback. The only must is a cmd property in the config argument. (More on config in config section).
...
tExecFile is a wrapper around execFile from child_process, it takes two argument, configs(array of config obejct) and the optional callback function. What is always required is the cmd and args properties in configs argument, where cmd is an enviroment in which to execute the file (node, python etc..), and args will take the file name. Let's see an example:
...
tFork is a wrapper around fork from child_process, it also takes two arguments configs(array of config obejct) and an optional callback function, whit the only required property in configs being cmd which will take the name of the file to be executed. Example:
...
<a name="tSpawn"></a>
tSpawn is a wrapper around spawn from child_process like all other termax wrappers it also takes two arguments, configs(array of config obejct) and an optional callback function, and it always required is the cmd and args properties.
Example:
...
Chain is a class allowing you to to chain together multiple diferent execution sequences. Let's say you want to execute three commands in a sequence and you choose tSpawn to do so, but then you would like to execute a file(maybe generated by one of the commands), now if you try to call tSpawn and right under it tFork, both these will start to execute almost simultaneously. The reason being that each execution in both tSpawn and tFork are async and as such they will execute side by side there for both execution sequence of tSpawn and tFork will execute side by side.
Chain fixes this by allowing to chain together these execution sequences. Lets see this in an example:
...
Or we can call give it as an argument in addToChain:
...
All termax wrappers come with built in error handling, by default it is turned off and you will simply be prompted that there was an error, but the exution sequence will continue:
...
Type: `string`
In tExec, tExecFile and tSpawn it takes the command to be executed in tFork it takes a filename to be executed.
...
Type: `boolean`
If set to true implements error handling (default false).
...
Type: `string`
Stes one of the prebuilt styles
...
<a name="themes"></a>
You can use a number of built in themes to speed up your work simply by adding a style property in spinner section fo the execution config:
...
If on the other hand you would like to have full control of the styling you could do that by choosing custom style and passing a style config:
